...
type SessionsAPI =
    ...
    :<|> Header "Cookie" SessionCookie :> Capture "session_id" SessionId :> "lock" :> Post '[JSON] ()
    :<|> Header "Cookie" SessionCookie :> Capture "session_id" SessionId :> Delete '[JSON] ()
    ...
    :<|> Header "Cookie" SessionCookie :> Capture "session_id" SessionId :> "questions" :> Capture "question_id" QuestionId :> "answer" :> Post '[JSON] ()
...
sessionsServer app = newSessionH :<|> lockSessionH :<|> deleteSessionH :<|> joinSessionH :<|> askQH :<|> upvoteQH :<|> answerQH
    where
        ...
        lockSessionH c id = withError <=< liftIO . sessionLockHandler app c $ id
        deleteSessionH c id = withError <=< liftIO . sessionDeleteHandler app c $ id
        ...
        answerQH cookie sessionId = withError <=< liftIO . answerQuestionHandler app cookie sessionId
